/*
 * Copyright (c) 2021 IBA Group, a.s. All rights reserved.
 *
 * Licensed to the Apache Software Foundation (ASF) under one or more
 * contributor license agreements.  See the NOTICE file distributed with
 * this work for additional information regarding copyright ownership.
 * The ASF licenses this file to You under the Apache License, Version 2.0
 * (the "License"); you may not use this file except in compliance with
 * the License.  You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import settingsUsersRolesReducer from './settingsUsersRolesReducer';
import {
    FETCH_USERS_AND_ROLES_START,
    FETCH_USERS_AND_ROLES_FAIL,
    FETCH_USERS_AND_ROLES_SUCCESS,
    UPDATE_USERS_AND_ROLES_FAIL,
    UPDATE_USERS_AND_ROLES_SUCCESS,
    UPDATE_USERS_AND_ROLES_START
} from '../actions/types';

describe('settings users and roles Reducer', () => {
    const payload = [
        { username: 'TTest', name: 'Tested Testing', grant: 'Administrator' }
    ];

    it('should return the initial state', () => {
        expect(settingsUsersRolesReducer(undefined, {})).toEqual({
            loading: true,
            data: {}
        });
    });

    it('should handle FETCH_USERS_AND_ROLES_SUCCESS', () => {
        const action = {
            type: FETCH_USERS_AND_ROLES_SUCCESS,
            payload
        };
        expect(settingsUsersRolesReducer(undefined, action)).toEqual({
            loading: false,
            data: payload
        });
    });

    it('should handle FETCH_USERS_AND_ROLES_FAIL', () => {
        const action = {
            type: FETCH_USERS_AND_ROLES_FAIL,
            payload: { error: {} }
        };
        expect(settingsUsersRolesReducer(undefined, action)).toEqual({
            loading: false,
            error: {},
            data: {}
        });
    });

    it('should handle FETCH_USERS_AND_ROLES_START', () => {
        const action = {
            type: FETCH_USERS_AND_ROLES_START
        };
        expect(settingsUsersRolesReducer(undefined, action)).toEqual({
            loading: true,
            data: {}
        });
    });

    it('should handle UPDATE_USERS_AND_ROLES_FAIL', () => {
        const action = {
            type: UPDATE_USERS_AND_ROLES_FAIL,
            payload: { error: {} }
        };
        expect(settingsUsersRolesReducer(undefined, action)).toEqual({
            error: {},
            data: {},
            loading: false
        });
    });

    it('should handle UPDATE_USERS_AND_ROLES_SUCCESS', () => {
        const action = {
            type: UPDATE_USERS_AND_ROLES_SUCCESS,
            payload
        };
        expect(settingsUsersRolesReducer(undefined, action)).toEqual({
            data: { grants: payload },
            loading: false
        });
    });

    it('should handle UPDATE_USERS_AND_ROLES_START', () => {
        const action = {
            type: UPDATE_USERS_AND_ROLES_START
        };
        expect(settingsUsersRolesReducer(undefined, action)).toEqual({
            loading: true,
            data: {}
        });
    });
});
